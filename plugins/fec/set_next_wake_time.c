...
static void set_next_wake_time_init(picoquic_cnx_t* cnx, uint64_t current_time) {
...
protoop_arg_t set_next_wake_time(picoquic_cnx_t *cnx)
{
    ...
    {
        set_next_wake_time_init(cnx, current_time);
        return 0;
    }
    ...
    if (blocked != 0) {
        for (int i = 0; blocked != 0 && pacing == 0 && i < cnx->nb_paths; i++) {
            ...
            for (picoquic_packet_context_enum pc = 0; pc < picoquic_nb_packet_context; pc++) {
                ...
                if (p != NULL && ret == 0 && helper_retransmit_needed_by_packet(cnx, p, current_time, /* &ph,*/ &timer_based)) {
                    blocked = 0;
                }
            ...
            if (blocked != 0) {
                if (path_x->cwin > path_x->bytes_in_transit) {
                    ...
                    if (helper_should_send_max_data(cnx) ||
                        ...
                        state->should_check_block_flush = false;
                    }
                }
            }
        }
