...
static __attribute__((always_inline)) uint8_t gf256_mul(uint8_t a, uint8_t b)
...
(uint8_t *symbol1, uint8_t coef, uint32_t symbol_size)
{
    for (uint32_t i=0; i<symbol_size; i++) {
        symbol1[i] = gf256_mul(coef, symbol1[i]);
    }
}
