...
protoop_arg_t protoop_log(picoquic_cnx_t *cnx) {
    TMP_FRAME_BEGIN(cnx, parsed_frame, frame, new_connection_id_frame_t)
        ...
        PROTOOP_SNPRINTF(cnx, frame_str, 400, "{\"frame_type\": \"new_connection_id\", \"sequence_number\": \"%" PRIu64 "\", \"retire_prior_to\": 0, \"length\": %d, \"connection_id\": \"%s\", \"reset_token\": \"\"}", frame.sequence, frame.connection_id.id_len, (protoop_arg_t) id_str);
