...
protoop_arg_t protoop_log(picoquic_cnx_t *cnx) {
    TMP_FRAME_BEGIN(cnx, parsed_frame, frame, application_close_frame_t)
        ...
        if (!frame_str)
            return 0;
        PROTOOP_SNPRINTF(cnx, frame_str, frame_str_len, "{\"frame_type\": \"connection_close\", \"error_space\": \"application\", \"raw_error_code\": %d, \"reason\": \"%s\"}", frame.error_code, (protoop_arg_t) (frame.reason_phrase_length > 0 ? frame.reason_phrase : ""));
