            ...
            if (should_retransmit == 0) {
                ...
                */
                ...
                if (should_retransmit == 2) {
                    p = (picoquic_packet_t *) get_pkt_ctx(orig_pkt_ctx, AK_PKTCTX_RETRANSMIT_NEWEST);
                }
                ...
                int is_evaluated = (int) get_pkt(p, AK_PKT_IS_EVALUATED);
                ...
                if (ptype == picoquic_packet_0rtt_protected) {
                    ...
                    if (is_evaluated == 0) {
                        ...
                        set_pkt(p, AK_PKT_CONTAINS_CRYPTO, contains_crypto);
                        set_pkt(p, AK_PKT_IS_PURE_ACK, packet_is_pure_ack);
                        set_pkt(p, AK_PKT_IS_EVALUATED, 1);
                    } else {
                        contains_crypto = get_pkt(p, AK_PKT_CONTAINS_CRYPTO);
                        packet_is_pure_ack = get_pkt(p, AK_PKT_IS_PURE_ACK);
                    }
