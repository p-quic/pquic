 ...
 * uint64_t path_id = cnx->protoop_inputv[4]
 *
...
protoop_arg_t update_rtt(picoquic_cnx_t *cnx)
{
    ...
    if (cnx->protoop_inputc > 4) {
        path_id = (uint64_t) cnx->protoop_inputv[4];
    }
    ...
    if (path_id && pc == picoquic_packet_context_application) {
        path_index = mp_get_path_index(bpfd, path_id, &new_path_index);
        if (path_index < 0 || new_path_index) {
            return (protoop_arg_t) packet;
        }
        pkt_ctx = &bpfd->paths[path_index].pkt_ctx;
        packet = pkt_ctx->retransmit_newest;
    }
