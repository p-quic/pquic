    ...
    TMP_FRAME_BEGIN(cnx, parsed_frame, frame, mp_new_connection_id_frame_t)
        {
            ...
            PROTOOP_SNPRINTF(cnx, frame_str, 200, "{\"frame_type\": \"mp_new_connection_id\", \"uniflow_id\": \"%" PRIu64 "\", \"sequence\": %" PRIu64 ", \"len\": %d, \"cid\": \"%s\"}", frame.uniflow_id, frame.ncidf.sequence, frame.ncidf.connection_id.id_len, (protoop_arg_t) id_str);
