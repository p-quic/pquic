    ...
    TMP_FRAME_BEGIN_MALLOC(cnx, parsed_frame, frame, mp_ack_frame_t)
        {
            ...
            for (int num_block = -1; num_block < ack_block_count; num_block++) {
                ...
                if (range <= 1)
                    ack_ofs += snprintf(block_str + ack_ofs, BLOCK_STR_LEN - ack_ofs, "[%lu]", largest);
                else
                    ack_ofs += snprintf(block_str + ack_ofs, BLOCK_STR_LEN - ack_ofs, "[%lu, %lu]", largest - range + 1, largest);
            ...
            if (!ack_str)
                return 0;
            PROTOOP_SNPRINTF(cnx, ack_str, BLOCK_STR_LEN + 200, "{\"frame_type\": \"mp_ack\", \"path_id\": \"%lu\", \"ack_delay\": \"%lu\", \"acked_ranges\": [%s]}", frame->path_id, frame->ack.ack_delay, (protoop_arg_t) block_str);
