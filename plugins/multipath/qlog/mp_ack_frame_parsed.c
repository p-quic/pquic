    ...
    TMP_FRAME_BEGIN_MALLOC(cnx, parsed_frame, frame, mp_ack_frame_t)
        {
            ...
            ack_str[ack_ofs] = 0;
            LOG_EVENT(cnx, "FRAMES", "MP_ACK_FRAME_PARSED", "", "{\"ptr\": \"%p\", \"path_id\": %lu, \"largest\": %lu, \"blocks\": [%s]}", (protoop_arg_t) parsed_frame, frame->path_id, frame->ack.largest_acknowledged, (protoop_arg_t) ack_str);
            my_free(cnx, ack_str);
        }
