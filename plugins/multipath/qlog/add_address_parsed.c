...
protoop_arg_t protoop_log(picoquic_cnx_t *cnx) {
    TMP_FRAME_BEGIN(cnx, parsed_frame, frame, add_address_frame_t)
        ...
        PROTOOP_SNPRINTF(cnx, frame_str, sizeof(addr_str) + 100, "{\"frame_type\": \"add_address\", \"address_id\": %lu, \"ip_vers\": %d, \"address\": \"%s\"}", frame.address_id, frame.ip_vers, (protoop_arg_t) addr_str);
