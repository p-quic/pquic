...
protoop_arg_t parse_mp_new_connection_id_frame(picoquic_cnx_t* cnx)
{
    ...
    if (!frame) {
        PROTOOP_PRINTF(cnx, "Failed to allocate memory for new_connection_id_frame_t\n");
    ...
    if ((bytes = picoquic_frames_varint_decode(bytes + picoquic_varint_skip(bytes), bytes_max, &frame->uniflow_id))  == NULL ||
        ...
        (bytes = picoquic_frames_varint_decode(bytes, bytes_max, &frame->ncidf.retire_prior_to))   == NULL ||
