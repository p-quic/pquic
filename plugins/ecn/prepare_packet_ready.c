...
static int prepare_ecn_frame(picoquic_cnx_t *cnx, uint8_t *bytes, size_t bytes_max,
    size_t *consumed)
{
    protoop_arg_t outs[1], args[3];
    args[0] = (protoop_arg_t) bytes;
    args[1] = (protoop_arg_t) bytes_max;
    args[2] = (protoop_arg_t) *consumed;
    int ret = (int) plugin_run_protoop(cnx, PROTOOPID_PREPARE_ECN_FRAME, 3, args, outs);
            ...
            if (((stream == NULL && tls_ready == 0 && cnx->first_misc_frame == NULL) || path_x->cwin <= path_x->bytes_in_transit)
                ...
                if (cnx->cnx_state != picoquic_state_disconnected) {
                    ...
                    if (path_x->cwin > path_x->bytes_in_transit) {
                        ...
                        /* if present, send ecn data */
                        if (prepare_ecn_frame(cnx, &bytes[length],
                            send_buffer_min_max - checksum_overhead - length, &data_bytes)) {
                            length += (uint32_t) data_bytes;
                        }
